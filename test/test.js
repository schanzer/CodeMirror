...
testCM("structuredFold", function(cm) {
  ...
  eqPos(cm.getCursor(), Pos(6, 2));
  CodeMirror.commands.goCharLeft(cm);
  eqPos(cm.getCursor(), Pos(6, 1, "after"));
  eq(cleared, 1);
  range.clear();
  eq(cleared, 1);
  range = cm.markText(Pos(1, 2), Pos(6, 2), {
  ...
  eqPos(cm.getCursor(), Pos(1, 3, "before"));
...
testCM("nestedFold", function(cm) {
  ...
  outer.clear();
  CodeMirror.commands.goCharRight(cm);
  eqPos(cm.getCursor(), Pos(0, 2, "before"));
  CodeMirror.commands.goCharRight(cm);
  eqPos(cm.getCursor(), Pos(1, 8));
  inner2.clear();
  CodeMirror.commands.goCharLeft(cm);
  eqPos(cm.getCursor(), Pos(1, 7, "after"));
  cm.setCursor(0, 5);
  CodeMirror.commands.goCharRight(cm);
  eqPos(cm.getCursor(), Pos(0, 6, "before"));
  CodeMirror.commands.goCharRight(cm);
...
testCM("wordMovementCommands", function(cm) {
  ...
  eqPos(cm.getCursor(), Pos(0, 7, "before"));
  cm.execCommand("goWordLeft");
  eqPos(cm.getCursor(), Pos(0, 5, "before"));
  cm.execCommand("goWordRight"); cm.execCommand("goWordRight");
  eqPos(cm.getCursor(), Pos(0, 12, "before"));
  cm.execCommand("goWordLeft");
  eqPos(cm.getCursor(), Pos(0, 9, "before"));
  cm.execCommand("goWordRight"); cm.execCommand("goWordRight"); cm.execCommand("goWordRight");
  eqPos(cm.getCursor(), Pos(0, 24, "before"));
  cm.execCommand("goWordRight"); cm.execCommand("goWordRight");
  eqPos(cm.getCursor(), Pos(1, 9, "before"));
  cm.execCommand("goWordRight");
  eqPos(cm.getCursor(), Pos(1, 13, "before"));
  cm.execCommand("goWordRight"); cm.execCommand("goWordRight");
  eqPos(cm.getCursor(), Pos(2, 0, "before"));
...
testCM("groupMovementCommands", function(cm) {
  ...
  eqPos(cm.getCursor(), Pos(0, 4, "before"));
  cm.execCommand("goGroupRight");
  eqPos(cm.getCursor(), Pos(0, 7, "before"));
  cm.execCommand("goGroupRight");
  eqPos(cm.getCursor(), Pos(0, 10, "before"));
  cm.execCommand("goGroupLeft");
  eqPos(cm.getCursor(), Pos(0, 7, "before"));
  cm.execCommand("goGroupRight"); cm.execCommand("goGroupRight"); cm.execCommand("goGroupRight");
  eqPos(cm.getCursor(), Pos(0, 15, "before"));
  cm.setCursor(Pos(0, 17));
  cm.execCommand("goGroupLeft");
  eqPos(cm.getCursor(), Pos(0, 16, "before"));
  cm.execCommand("goGroupLeft");
  eqPos(cm.getCursor(), Pos(0, 14, "before"));
  cm.execCommand("goGroupRight"); cm.execCommand("goGroupRight");
  eqPos(cm.getCursor(), Pos(0, 20, "before"));
  cm.execCommand("goGroupRight");
  eqPos(cm.getCursor(), Pos(1, 0, "before"));
  cm.execCommand("goGroupRight");
  eqPos(cm.getCursor(), Pos(1, 2, "before"));
  cm.execCommand("goGroupRight");
  eqPos(cm.getCursor(), Pos(1, 5, "before"));
  cm.execCommand("goGroupLeft"); cm.execCommand("goGroupLeft");
  eqPos(cm.getCursor(), Pos(1, 0, "after"));
  cm.execCommand("goGroupLeft");
  eqPos(cm.getCursor(), Pos(0, 20, "before"));
  cm.execCommand("goGroupLeft");
  eqPos(cm.getCursor(), Pos(0, 16, "before"));
...
testCM("groupsAndWhitespace", function(cm) {
  ...
  for (var i = 1; i < positions.length; i++) {
    cm.execCommand("goGroupRight");
    positions[i].sticky = "before";
    eqPos(cm.getCursor(), positions[i]);
  }
  for (var i = positions.length - 2; i >= 0; i--) {
    cm.execCommand("goGroupLeft");
    positions[i].sticky = positions[i].ch == 0 ? "after" : "before";
    eqPos(cm.getCursor(), i == 2 ? Pos(0, 6, "before") : positions[i]);
  }
}, {value: "  foo +++  \n  bar"});
...
testCM("rtlMovement", function(cm) {
  ...
  forEach(["خحج", "خحabcخحج", "abخحخحجcd", "abخde", "abخح2342خ1حج", "خ1ح2خح3حxج",
    ...
    var inv = false && (line.charCodeAt(0) > 128);
    ...
    for (var i = 0; i < line.length; ++i) {
      cm.execCommand("goCharLeft");
      cursor = cursors.firstChild;
console.log(i, line.length, { l: cursor.offsetLeft }, { l: prevX } )
...
testCM("movebyTextUnit", function(cm) {
  ...
  eqPos(cm.getCursor(), Pos(0, 0, "after"));
  cm.execCommand("goCharRight");
  eqPos(cm.getCursor(), Pos(1, 0, "before"));
  cm.execCommand("goCharRight");
  cm.execCommand("goCharRight");
  eqPos(cm.getCursor(), Pos(1, 4, "before"));
  cm.execCommand("goCharRight");
  eqPos(cm.getCursor(), Pos(1, 7, "before"));
});
...
testCM("atomicMarker", function(cm) {
  ...
  cm.execCommand("goCharLeft");
  eqPos(cm.getCursor(), Pos(8, 3, "after"));
  m.clear();
  m = atom(1, 1, 3, 8);
  cm.setCursor(Pos(0, 0));
  cm.setCursor(Pos(2, 0));
...
testCM("addKeyMap", function(cm) {
  ...
  sendKey(39);
  eqPos(cm.getCursor(), Pos(0, 1, "before"));
  var test = 0;
  ...
  sendKey(39);
  eqPos(cm.getCursor(), Pos(0, 3, "before"));
}, {value: "abc"});
...
function makeItWrapAfter(cm, pos) {
  ...
  var secondLineTop, posTop;
  var w = 0;
  do {
    secondLineTop = posTop;
    cm.setSize(++w);
    posTop = cm.charCoords(pos).top;
  } while (posTop != firstLineTop)
  return [firstLineTop, secondLineTop];
}

testCM("positions", function(cm) {
  var linesTop = makeItWrapAfter(cm, Pos(0, 2));
  is(linesTop[1] > linesTop[0]);
  var lastLeft = -1, targetTop = linesTop[0];
